...
function getQueenSimpleMoves(board, row, col) {
  const moves = [];

  for (const [dRow, dCol] of MOVE_DIRECTIONS.king) {
    let toRow = row + dRow;
    let toCol = col + dCol;

    while (isInsideBoard(toRow, toCol) && !board[toRow][toCol]) {
      moves.push({
        from: { row, col },
...
function getSimpleMoves(board, row, col, piece) {
  if (piece.king) {
    return getQueenSimpleMoves(board, row, col);
  }
...
function getManCaptureMoves(board, row, col, piece) {
  const moves = [];
...
function getQueenCaptureMoves(board, row, col, piece) {
  const moves = [];
  ...
  for (const [dRow, dCol] of MOVE_DIRECTIONS.king) {
    ...
    while (isInsideBoard(scanRow, scanCol)) {
      ...
      if (!captured) {
        ...
        if (scannedPiece.player === piece.player) {
          break;
        }
...
export function getCaptureMoves(board, row, col, piece) {
  return piece.king
    ? getQueenCaptureMoves(board, row, col, piece)
    : getManCaptureMoves(board, row, col, piece);
}
